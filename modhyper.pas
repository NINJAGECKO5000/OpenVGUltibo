...
uses GlobalConfig,GlobalConst,GlobalTypes,Platform,Threads,Devices,GPIO,I2C,Touch,I2CGPIO,GoodixTouch,FT5x06Touch,SysUtils,PWM;
...
function HyperPixelBacklight(Backlight : integer):LongWord;
...
begin
 ...
 Result:=ERROR_INVALID_PARAMETER;
	{Setup Backlight GPIO for PWM}
	GPIOBacklight:= PWMDeviceFindByName('PWM1');{some logic would need to be setup here for determining diffrent boards,
        im not smart enough for that so this is hardcoded for a pi Zero W}
	PWMDeviceSetRange(GPIOBacklight, 100);{Set the range to 255}
	PWMDeviceSetMode(GPIOBacklight,PWM_MODE_MARKSPACE);{And the mode to PWM_MODE_MARKSPACE}
	PWMDeviceSetFrequency(GPIOBacklight,9600000);{set the frequency to 9.6MHz}
	PWMDeviceStart(GPIOBacklight); {Start backlight PWM}
	PWMDeviceWrite(GPIOBacklight,100);{Set backlight to full brightness as a default}
 {Check Model}
...
function HyperPixelBacklight(Backlight : integer):LongWord;
begin
	if (Backlight > 100) or (Backlight < 0) then Exit;  {my C++ Logic doesent work here}
	PWMDeviceWrite(GPIOBacklight, Backlight);
end;
...
begin
 ...
 IRQ.GPIO:=GPIODevice;
 IRQ.Pin:=HYPERPIXEL_TOUCH_INTERRUPT_PIN;
 IRQ.Func:=HYPERPIXEL_TOUCH_INTERRUPT_FUNCTION;
 IRQ.Pull:=HYPERPIXEL_TOUCH_INTERRUPT_PULL;
 IRQ.Trigger:=HYPERPIXEL_TOUCH_INTERRUPT_TRIGGER;

 {Setup Touch Device Parameters}
 GOODIX_SWAP_CONFIG_XY:=True;
 GOODIX_SWAP_REPORT_XY:=True;
 GOODIX_INVERT_REPORT_X:=True;
...
begin
 ...
 HyperPixelSPIWriteCommand(@Data,$100 or $29);
 HyperPixelSPIWriteCommand(@Data,$100 or $12);
 HyperPixelSPIWriteCommand(@Data,$100 or $17);
 HyperPixelSPIWriteCommand(@Data,$100 or $0B);
 HyperPixelSPIWriteCommand(@Data,$100 or $18);
 HyperPixelSPIWriteCommand(@Data,$100 or $09);
 HyperPixelSPIWriteCommand(@Data,$100 or $08);
 HyperPixelSPIWriteCommand(@Data,$100 or $2A);
 HyperPixelSPIWriteCommand(@Data,$100 or $07);
 HyperPixelSPIWriteCommand(@Data,$100 or $14);
 HyperPixelSPIWriteCommand(@Data,$100 or $11);
 HyperPixelSPIWriteCommand(@Data,$100 or $27);
 HyperPixelSPIWriteCommand(@Data,$100 or $32);
 ...
 HyperPixelSPIWriteCommand(@Data,$100 or $29);
 HyperPixelSPIWriteCommand(@Data,$100 or $12);
 HyperPixelSPIWriteCommand(@Data,$100 or $16);
 ...
 HyperPixelSPIWriteCommand(@Data,$100 or $9D);  {6D OR 5D}

 HyperPixelSPIWriteCommand(@Data,$B1);  {VCOM amplitude setting}
 HyperPixelSPIWriteCommand(@Data,$100 or $24);  {37 OR 43}
 ...
 HyperPixelSPIWriteCommand(@Data,$100 or $3F);
 HyperPixelSPIWriteCommand(@Data,$100 or $1F);
 ...
 HyperPixelSPIWriteCommand(@Data,$E8);
 HyperPixelSPIWriteCommand(@Data,$100 or $00);
 HyperPixelSPIWriteCommand(@Data,$100 or $0C);
